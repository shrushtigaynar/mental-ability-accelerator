...
import { getLogicQuestions, submitTestResults, TRAINING_TOPICS } from "../api/practice.js";
...
export default function LogicTraining() {
  ...
  const topics = TRAINING_TOPICS.interview;
  ...
  // Handle timer reaching 0 - show timeout feedback before advancing
  useEffect(() => {
    if (timeLeft === 0 && currentStep === 3 && !loading && questions.length > 0) {
      // Mark as timed out but don't advance immediately
  ...
  }, [timeLeft, currentStep, loading, questionIndex, questions.length]);
...
const getOptionStyle = (idx) => {
  ...
  const getQuestionStatus = (qIndex) => {
    ...
    const answer = answers.find(a => a.questionId === (questions[qIndex]?.id || qIndex));
    if (answer) {
      if (answer.selectedOption === null) return 'timeout';
  ...
  const loadQuestions = async () => {
    ...
    try {
      const data = await getLogicQuestions(token, selectedTopic?.name, selectedDifficulty);
  ...
  const wrongCount = answers.filter(a => !a.is_correct && a.selected_option !== null).length;
  const timeoutCount = answers.filter(a => !a.is_correct && a.selected_option === null).length;
  const accuracyPct = answers.length > 0 ? Math.round((correctCount / answers.length) * 100) : 0;
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-xl font-semibold tracking-tight text-white sm:text-2xl">
              {currentStep === 1 && "Logical Reasoning"}
            ...
            {topics.map((topic) => (
              ...
              >
                ...
                <button
                  ...
                  className="mt-4 w-full rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-emerald-500/30 transition hover:from-emerald-400 hover:to-teal-400"
                >
                  Start Practice →
                </button>
              </div>
            ...
            {loading ? (
              ...
              <div className="mt-10 space-y-4">
                ...
                <div className="mt-4 flex gap-3">
                  <button
                    type="button"
                    className="flex-1 inline-flex items-center justify-center rounded-full bg-gradient-to-r from-emerald-500 to-teal-500 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-emerald-500/30 transition hover:from-emerald-400 hover:to-teal-400"
                    onClick={() => navigate("/dashboard")}
                  >
                    Back to Dashboard
                  </button>
              ...
              <div className="flex gap-6 mt-8">
                  ...
                  <div className="flex items-center gap-3">
                    <div className={`flex h-9 w-14 items-center justify-center rounded-full border text-sm font-semibold transition ${
                      timeLeft <= 10 
                        ...
                        : 'border-emerald-500/60 bg-slate-900/80 text-emerald-200'
                    }`}>
                      {timeLeft}s
                    </div>
                  </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {currentOptions.map((option, idx) => {
                    ...
                    const selectedStyle = isSelected ? " border-blue-400/80 bg-blue-600 text-white" : " hover:border-emerald-400/80 hover:text-emerald-200";
